...
interface GenderForecastProps {
  data: any[];
}

export default function GenderForecast({ data }: GenderForecastProps) {
  ...
  // Training State
  // CHANGED: Renamed selectedStatus to selectedGender for clarity
  ...
  const MODEL_NAME = 'gender-distri-mlp';

  // CHANGED: Renamed statusOptions to genderOptions
  const genderOptions = useMemo(() => {
    if (!data || data.length === 0) return [];
    // Filters out non-numeric keys. Assumes keys are 'Male', 'Female', etc.
  ...
  const generateForecast = async (loadedModel: any, meta: any) => {
    ...
    try {
      // CHANGED: Changed targetStatus to targetGender to match new metadata structure
      const targetKey = meta.targetGender; 
  ...
  const handleTrain = async () => {
    ...
    try {
      // CHANGED: Using selectedGender
      const targetKey = selectedGender; 
      ...
      const newMetadata = {
        // CHANGED: Saving as targetGender instead of targetStatus
        targetGender: targetKey, 
  ...
  const handleUploadModel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (metadataFile) {
        ...
        // CHANGED: Update prompt text to match Gender context
        const manualInput = prompt("Metadata missing. Enter Target Gender (e.g. Female):");
        if (!manualInput) throw new Error("Metadata required.");
        // CHANGED: Create targetGender key
        loadedMetadata = { targetGender: manualInput, lookback: 3, min: 0, max: 1000 };
      }
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-xl font-bold text-amber-900 flex items-center gap-2">
            ...
            Gender Distribution Forecasting
          </h3>
          <p className="text-sm text-amber-700/60">
            {/* CHANGED: Display targetGender */}
            {metadata ? `Active: ${metadata.targetGender} (Lookback: ${metadata.lookback} yrs)` : 'No model loaded'}
          </p>
        </div>
            ...
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={forecastData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Legend />
                  {/* CHANGED: DataKey uses targetGender */}
                  <Line type="monotone" dataKey={metadata?.targetGender} stroke="#d97706" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 8 }} />
                </LineChart>
              </ResponsiveContainer>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-amber-50 p-6 rounded-xl border border-amber-100">
              ...
              <div className="space-y-4 mb-6">
                {/* Target Selection */}
                <div>
                    <label className="block text-sm font-bold text-amber-800 mb-1">Target Gender</label>
                    {/* CHANGED: Use selectedGender and genderOptions */}
                    <select value={selectedGender} onChange={(e) => setSelectedGender(e.target.value)} className="w-full p-3 border border-amber-200 rounded-lg bg-white focus:ring-2 focus:ring-amber-500 outline-none">
